angular.module('testx', ['wilddog']);

describe("wild-angularAuth Test Suite", function() {
  ...
  beforeEach(function() {
    ...
    if(ngFireRef == null) {
      module("testx")
      inject(function(_$wilddog_, _$wilddogSimpleLogin_, _$timeout_, _$rootScope_) {
        $wilddog = _$wilddog_;
        $wilddogSimpleLogin = _$wilddogSimpleLogin_;
        $timeout = _$timeout_;
        $rootScope = _$rootScope_;
        var ref = new Wilddog("https://angularfiretests.wilddogio.com");
        ngFireRef = $wilddog(ref);
        ngSimpleLogin = $wilddogSimpleLogin(ref);
  ...
  it("Email: failed login", function(done) {
    ...
    var off = $rootScope.$on("$wilddogSimpleLogin:error", function(event, err) {
      expect(err).not.toBe(null);
  ...
  it("Successful Twitter login", function(done) {
    ...
    ngSimpleLogin.$getCurrentUser().then(function() {
      var off = $rootScope.$on("$wilddogSimpleLogin:login", function(event, user) {
        expect(user).not.toBe(null);
  ...
  it("Logout", function(done) {
    ...
    var off = $rootScope.$on("$wilddogSimpleLogin:logout", function(event) {
      waiter.done("event");
      off();
    });
