'use strict';
describe('$wilddogArray', function () {
  ...
  var arr, $wilddogArray, $utils, $timeout, testutils;
  beforeEach(function() {
    module('wilddog');
    module('testutils');
    inject(function (_$wilddogArray_, $wilddogUtils, _$timeout_, _testutils_) {
      testutils = _testutils_;
      $timeout = _$timeout_;
      $wilddogArray = _$wilddogArray_;
      $utils = $wilddogUtils;
  ...
  describe('<constructor>', function() {
    beforeEach(function() {
      inject(function($wilddogUtils, $wilddogArray) {
        this.$utils = $wilddogUtils;
        this.$wilddogArray = $wilddogArray;
      });
    });
    ...
    it('should have API methods', function() {
      var i = 0;
      this.$utils.getPublicMethods($wilddogArray, function(v,k) {
  ...
  describe('$add', function() {
    it('should call $push on $wilddog', function() {
      ...
      function addPromise(snap, prevChild){
        return new $utils.promise(
          function(resolve) {
            ...
            var data = $wilddogArray.prototype.$$added.call(arr, snap, prevChild);
            data.name = name;
            return data;
          });
      }
      arr = stubArray(null, $wilddogArray.$extend({$$added:addPromise}));
    ...
    it('should wait to resolve $loaded until $$added promise is resolved', function () {
      ...
      var called = false;
      var ref = stubRef();
      arr = stubArray(null, $wilddogArray.$extend({$$added:addPromise}), ref);
  ...
  describe('$remove', function() {
    it('should call remove on Wilddog ref', function() {
      var key = arr.$keyAt(1);
    ...
    it('should work on a query', function() {
      ...
      var query = ref.limit(5); //todo-mock MockWilddog does not support 2.x queries yet
      var arr = $wilddogArray(query);
      flushAll(arr.$ref());
      var key = arr.$keyAt(1);
      arr.$remove(1).then(whiteSpy, blackSpy);
  ...
  describe('$ref', function() {
    it('should return Wilddog instance it was created with', function() {
      var ref = stubRef();
      var arr = $wilddogArray(ref);
  ...
  describe('$$added', function() {
    ...
    it('should apply $$defaults if they exist', function() {
      var arr = stubArray(null, $wilddogArray.$extend({
  ...
  describe('$$updated', function() {
    ...
    it('should apply $$defaults if they exist', function() {
      var arr = stubArray(STUB_DATA, $wilddogArray.$extend({
        $$defaults: {aString: 'not_applied', foo: 'foo'}
      }));
  ...
  describe('$$error', function() {
    it('should call $destroy', function() {
      ...
      var arr = stubArray(STUB_DATA, $wilddogArray.$extend({ $destroy: spy }));
      spy.calls.reset();
      arr.$$error('test_err');
      expect(spy).toHaveBeenCalled();
    });
  ...
  describe('$$process', function() {
    ...
    it('should invoke $$notify with "child_added" event', function() {
      ...
      var arr = stubArray(STUB_DATA, $wilddogArray.$extend({ $$notify: spy }));
      spy.calls.reset();
      var rec = arr.$$added(testutils.snap({hello: 'world'}, 'addFirst'), null);
      arr.$$process('child_added', rec, null);
      expect(spy).toHaveBeenCalled();
  ...
  describe('$extend', function() {
    ...
    it('should return child class', function() {
      function A() {}
      var res = $wilddogArray.$extend(A);
      expect(res).toBe(A);
    });

    it('should be instanceof $wilddogArray', function() {
      function A() {}
      $wilddogArray.$extend(A);
      expect(new A(stubRef()) instanceof $wilddogArray).toBe(true);
    });

    it('should add on methods passed into function', function() {
      function foo() { return 'foo'; }
      var F = $wilddogArray.$extend({foo: foo});
      var res = F(stubRef());
    ...
    it('should work with the new keyword', function() {
      var fn = function() {};
      var Res = $wilddogArray.$extend({foo: fn});
  ...
  function stubRef() {
    return new MockWilddog('Mock://').child('data/REC1');
  }

  function stubArray(initialData, Factory, ref) {
    if( !Factory ) { Factory = $wilddogArray; }
    if( !ref ) {
      ref = stubRef();
    }
