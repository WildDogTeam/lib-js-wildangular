describe('WilddogAuth',function(){
  ...
  var $wilddogAuth, ref, auth, result, failure, status, $timeout, log;
  ...
  beforeEach(function(){
    ...
    module('wilddog',function($provide){
      $provide.value('$log',{
        warn:function(){
    ...
    inject(function(_$wilddogAuth_,_$timeout_){
      $wilddogAuth = _$wilddogAuth_;
      auth = $wilddogAuth(ref);
      $timeout = _$timeout_;
    });
  ...
  function getArgIndex(callbackName){
    //In the wilddog API, the completion callback is the second argument for all but a few functions.
    switch (callbackName){
  ...
  it('will throw an error if a string is used in place of a Wilddog Ref',function(){
    expect(function(){
  ...
  describe('$onAuth()',function(){
    //todo add more testing here after mockwilddog v2 auth is released
