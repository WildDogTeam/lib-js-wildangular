...
describe('TicTacToe App', function () {
  ...
  it('loads', function () {
    expect(browser.getTitle()).toEqual('wild-angular TicTacToe e2e Test');
  });
  ...
  it('persists state across refresh', function(done) {
    ...
    browser.get('tictactoe/tictactoe.html?pushId=' + wilddogRef.key()).then(function() {
      // Wait for wild-angular to sync the initial state
      sleep();
      sleep();
  ...
  it('stops updating Wilddog once the wild-angular bindings are destroyed', function () {
    // Make sure the board has 9 cells
    expect(cells.count()).toBe(9);
    ...
    flow.execute(function() {
      ...
      wilddogRef.child('x1/y2').once('value', function (dataSnapshot) {
