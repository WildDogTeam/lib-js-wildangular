...
(function() {
   ...
   *
   * Internally, the $wilddog object depends on this class to provide several $$ (i.e. protected)
   ...
   * the array class by returning a clone of $wilddogArray.
   *
   * <pre><code>
   * var ExtendedArray = $wilddogArray.$extend({
   ...
   * </code></pre>
   */
  angular.module('wilddog').factory('$wilddogArray', ["$log", "$wilddogUtils", "$q",
    function($log, $wilddogUtils, $q) {
      /**
       * This constructor should probably never be called manually. It is used internally by
       * <code>$wilddog.$asArray()</code>.
       *
       * @param {Wilddog} ref
       * @returns {Array}
       * @constructor
       */
      function WilddogArray(ref) {
        if( !(this instanceof WilddogArray) ) {
          return new WilddogArray(ref);
        }
        var self = this;
        this._observers = [];
        this.$list = [];
        ...
        $wilddogUtils.assertValidRef(ref, 'Must pass a valid Wilddog reference ' +
        'to $wilddogArray (not a string or URL)');
        ...
        // However, it's still possible to extend WilddogArray and have the public methods
        ...
        $wilddogUtils.getPublicMethods(self, function(fn, key) {
      ...
      WilddogArray.prototype = {
         ...
         * The resolve callback will be passed a Wilddog ref representing the new data element.
         *
         * @param data
        ...
        $add: function(data) {
          ...
          ref.set($wilddogUtils.toJSON(data), $wilddogUtils.makeNodeResolver(def));
         ...
         * Wilddog key (record.$id) for that record. If passed an invalid key or an item which
        ...
        /**
         * The inverse of $keyAt, this method takes a Wilddog key (record.$id) and returns the
        ...
        /**
         * @returns {Wilddog} the original Wilddog ref used to create this object.
         */
        ...
        /**
         * Returns the record for a given Wilddog key (record.$id). If the record is not found
         * then returns null.
         *
         * @param {string} key
        ...
        $$added: function(snap/*, prevChild*/) {
          ...
          var i = this.$indexFor($wilddogUtils.getKey(snap));
          if( i === -1 ) {
            ...
            rec.$id = $wilddogUtils.getKey(snap);
            rec.$priority = snap.getPriority();
            $wilddogUtils.applyDefaults(rec, this.$$defaults);
        ...
        $$removed: function(snap) {
          return this.$indexFor($wilddogUtils.getKey(snap)) > -1;
        },
        ...
        $$updated: function(snap) {
          var changed = false;
          var rec = this.$getRecord($wilddogUtils.getKey(snap));
          if( angular.isObject(rec) ) {
            // apply changes to the record
            changed = $wilddogUtils.updateRec(rec, snap);
            $wilddogUtils.applyDefaults(rec, this.$$defaults);
          }
          return changed;
        },
         ...
         * which tries to write data back to $wilddog.
         * @param {string} method
         * @private
         */
        _assertNotDestroyed: function(method) {
          if( this._isDestroyed ) {
            throw new Error('Cannot call ' + method + ' method on a destroyed $wilddogArray object');
          }
        }
      };

      /**
       * This method allows WilddogArray to be inherited by child classes. Methods passed into this
       ...
       * WilddogArray. It's also possible to do both, passing a class to inherit and additional
       ...
       * });
       *
       * // use our new factory in place of $wilddogArray
       * var list = new ExtendedArray(ref);
       * </code></pre>
       *
       * @param {Function} [ChildClass] a child class which should inherit WilddogArray
       * @param {Object} [methods] a list of functions to add onto the prototype
       * @returns {Function} a child class suitable for use with $wilddog (this will be ChildClass if provided)
       * @static
       */
      WilddogArray.$extend = function(ChildClass, methods) {
        if( arguments.length === 1 && angular.isObject(ChildClass) ) {
          ...
          ChildClass = function(ref) {
            ...
            WilddogArray.apply(this, arguments);
            return this.$list;
          };
        }
        return $wilddogUtils.inherit(ChildClass, WilddogArray, methods);
      };

      function ArraySyncManager(wilddogArray) {
        function destroy(err) {
          if( !sync.isDestroyed ) {
            sync.isDestroyed = true;
            var ref = wilddogArray.$ref();
        ...
        function init($list) {
          ...
          // listen for changes at the Wilddog instance
          ...
          ref.once('value', function(snap) {
            if (angular.isArray(snap.val())) {
              $log.warn('Storing data using array indices in Wilddog can result in unexpected behavior. See https://www.wilddog.com/docs/web/guide/understanding-data.html#section-arrays-in-wilddog for more information.');
            }
        ...
        var created = function(snap, prevChild) {
          waitForResolution(wilddogArray.$$added(snap, prevChild), function(rec) {
            wilddogArray.$$process('child_added', rec, prevChild);
          });
        };
        var updated = function(snap) {
          var rec = wilddogArray.$getRecord($wilddogUtils.getKey(snap));
          if( rec ) {
            waitForResolution(wilddogArray.$$updated(snap), function() {
              wilddogArray.$$process('child_changed', rec);
            });
          }
        };
        var moved   = function(snap, prevChild) {
          var rec = wilddogArray.$getRecord($wilddogUtils.getKey(snap));
          if( rec ) {
            waitForResolution(wilddogArray.$$moved(snap, prevChild), function() {
              wilddogArray.$$process('child_moved', rec, prevChild);
            });
          }
        };
        var removed = function(snap) {
          var rec = wilddogArray.$getRecord($wilddogUtils.getKey(snap));
          if( rec ) {
            waitForResolution(wilddogArray.$$removed(snap), function() {
               wilddogArray.$$process('child_removed', rec);
            });
          }
        };
  ...
  /** @deprecated */
  angular.module('wilddog').factory('$WilddogArray', ['$log', '$wilddogArray',
    function($log, $wilddogArray) {
      return function() {
        $log.warn('$WilddogArray has been renamed. Use $wilddogArray instead.');
        return $wilddogArray.apply(null, arguments);
      };
    }
  ]);
})();
...
(function() {
  ...
  angular.module('wilddog')
    .factory('$wilddogConfig', ["$wilddogArray", "$wilddogObject", "$injector",
      function($wilddogArray, $wilddogObject, $injector) {
        return function(configOpts) {
          ...
          return angular.extend({
            arrayFactory: $wilddogArray,
            objectFactory: $wilddogObject
          }, opts);
        };
      }
    ])

    .factory('$wilddogUtils', ["$q", "$timeout", "$rootScope",
        ...
        var utils = {
          ...
          assertValidRef: function(ref, msg) {
            if( !angular.isObject(ref) ||
              ...
              throw new Error(msg || 'Invalid Wilddog reference');
            }
          },
          ...
          /**
           * A utility for retrieving a Wilddog reference or DataSnapshot's
           * key name. This is backwards-compatible with `name()` from Wilddog
           * 1.x.x and `key()` from Wilddog 2.0.0+. Once support for Wilddog
           * 1.x.x is dropped in wild-angular, this helper can be removed.
           */
           ...
           * which we can save into Wilddog. It asserts valid
